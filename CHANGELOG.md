...
+ [新增] GitHub Actions 增加 PR 质量门、GHCR 双镜像构建与生产环境自动部署，并在迁移前执行可恢复备份、健康检查和失败回滚。

- [修复] 模型能力目录现在只补全缺失的视频与生图能力字段，渠道返回的具体字段仍优先，并正确隐藏不支持的生视频参数。

+ [新增] 新增 PostgreSQL 账户服务、邮箱注册登录与 sub2api OIDC 登录接入骨架。
+ [新增] 登录后可将浏览器中的画布、我的素材、生成记录与引用媒体一次性导入 PostgreSQL 和私有对象存储。
+ [新增] Docker 部署增加独立 MinIO、显式数据库迁移及 PostgreSQL/对象存储备份恢复验证。
+ [修复] 备份恢复校验等待临时 PostgreSQL 完成初始化后再导入，避免误连初始化阶段的临时进程。
+ [调整] Docker Web 端口默认仅监听本机，并补充 `canvas.zgonline.top` 生产域名配置。
+ [调整] 网站品牌更新为「织境」，重做 Logo、浏览器标题、首页、顶栏与登录界面，并移除用户可见的 GitHub 跳转入口。
+ [优化] 登录页增加织境账户与 Token 小铺账户边界说明，并将常见登录错误转换为本地化提示。
+ [修复] Token 小铺 OIDC 登录在授权接口返回后显式进入安全的授权地址，避免按钮无响应。
+ [调整] 默认 OpenAI 兼容渠道的 Base URL 改为 `https://api.zgonline.top/`。
+ [修复] 生图和视频工作台持久化当前生成任务，切换页面后可继续查看生成中、成功或失败状态。
+ [新增] 模型渠道增加隔离的能力适配层，视频创作台可按模型动态展示生成方式、分辨率、比例、时长和参考素材入口，普通 Sub2API 渠道保持原有调用路径。
